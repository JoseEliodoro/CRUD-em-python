...
class MyList(CTkFrame):
    ...
    def new_button_edit(self, master, id):
        return CTkButton(master, text='', image=self.img_edit, width=12, cursor='hand2', fg_color=bg_color_button_edit, hover_color=bg_hover_color_button,
                         command=lambda *a: self.edit_reg(id)).pack(side=LEFT)
    
    def new_button_excluir(self, master, id):
        return CTkButton(master, text='', image=self.img_delete, width=12, cursor='hand2', fg_color=bg_color_button_delete, hover_color=bg_hover_color_button,
                         command=lambda *a: self.excluirasdf(master, id)).pack(side=LEFT)
    
    def new_button_descricao(self, master, id):
        return CTkButton(master, text='', image=self.img_descri, width=12, cursor='hand2', fg_color=bg_color_button_describle, hover_color=bg_hover_color_button,
